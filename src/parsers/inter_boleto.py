...
_RE_VALOR = re.compile(r"Pagamento efetuado\s*\n?\s*R\$\s*([\d.,]+)", re.IGNORECASE)
_RE_NOME = re.compile(r"Nome\s+(.+)", re.IGNORECASE)


class InterBoletoParser(BaseParser):
    nome = "inter_boleto"

    def detect(self, texto: str) -> bool:
        return "Internet Banking Inter Empresas" in texto and "Pagamento efetuado" in texto
